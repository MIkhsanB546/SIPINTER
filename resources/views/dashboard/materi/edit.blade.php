            ...
            <form action="{{ route('dashboard.materi.update', $materi) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <select name="id_tingkat" id="id_tingkat" class="form-select @error('id_tingkat') is-invalid @enderror">
                        ...
                        <option value="{{ $tingkat->id_tingkat }}" {{ old('id_tingkat', $materi->id_tingkat) == $tingkat->id_tingkat ? 'selected' : '' }}>
...
function getPdfFormData() {
    ...
    if (fileInput.files.length > 0) {
        ...
        formData.append('materi_id', '{{ $materi->id_materi }}');
    }
...
async function submitGenerateQuiz() {
    ...
    try {
        ...
        if (result.success) {
            ...
            const quizData = {
                judul: 'Quiz ' + (document.getElementById('judul').value || '{{ $materi->judul }}'),
                durasi_menit: 30,
                deskripsi: '',
                soal: soalList,
            };
...
document.addEventListener('DOMContentLoaded', function () {
    @if ($materi->relationLoaded('quiz') && $materi->quiz->isNotEmpty())
    @php
        $firstQuiz = $materi->quiz->first();
        $existingQuizData = [
            'judul' => $firstQuiz->judul,
            'durasi_menit' => $firstQuiz->durasi_menit,
            'deskripsi' => $firstQuiz->deskripsi,
            'soal' => $firstQuiz->soal->map(function ($soal) {
                return [
                    'pertanyaan' => $soal->pertanyaan,
                    'options' => $soal->pilihanJawaban->pluck('jawaban')->toArray(),
                    'jawaban_benar' => $soal->pilihanJawaban->search(function ($p) { return $p->is_correct; }),
                    'penjelasan' => '',
                ];
            })->toArray(),
        ];
    @endphp
    const existingQuiz = @json($existingQuizData);
    if (existingQuiz && existingQuiz.soal && existingQuiz.soal.length > 0) {
        document.getElementById('quiz_data').value = JSON.stringify(existingQuiz);
        renderQuiz(existingQuiz);
        document.getElementById('quizSection').style.display = 'block';
    }
    @else
